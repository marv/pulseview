...
Ruler::Ruler(View &parent) : MarginWidget(parent)
{
	setMouseTracking(true);

	connect(&view_, SIGNAL(hover_point_changed()), this,
		SLOT(hover_point_changed()));
	connect(&view_, SIGNAL(offset_changed()), this,
		SLOT(invalidate_tick_position_cache()));
	connect(&view_, SIGNAL(scale_changed()), this,
		SLOT(invalidate_tick_position_cache()));
	connect(&view_, SIGNAL(tick_prefix_changed()), this,
		SLOT(invalidate_tick_position_cache()));
	connect(&view_, SIGNAL(tick_precision_changed()), this,
		SLOT(invalidate_tick_position_cache()));
	connect(&view_, SIGNAL(tick_period_changed()), this,
		SLOT(invalidate_tick_position_cache()));
	connect(&view_, SIGNAL(time_unit_changed()), this,
		SLOT(invalidate_tick_position_cache()));
}
...
QString Ruler::format_time_with_distance(const pv::util::Timestamp &distance,
	const pv::util::Timestamp &t, pv::util::SIPrefix prefix,
	pv::util::TimeUnit unit, unsigned precision, bool sign)
{
	...
	if (unit == pv::util::TimeUnit::Samples)
		return pv::util::format_time_si_adjusted(
			t, prefix, precision, "sa", sign);
...
void Ruler::paintEvent(QPaintEvent *)
{
	if (!tick_position_cache_) {
		...
		tick_position_cache_ =
			calculate_tick_positions(view_.tick_period(),
				view_.offset(), view_.scale(), width(), ffunc);
	}
...
Ruler::TickPositions Ruler::calculate_tick_positions(
	...
	const pv::util::Timestamp &offset, const double scale, const int width,
	std::function<QString(const pv::util::Timestamp &)> format_function)
{
	TickPositions tp;

	const pv::util::Timestamp minor_period =
		major_period / MinorTickSubdivision;
	const pv::util::Timestamp first_major_division =
		floor(offset / major_period);
	const pv::util::Timestamp first_minor_division =
		ceil(offset / minor_period);
...
{
	view_.add_flag(
		view_.offset() + ((double)event->x() + 0.5) * view_.scale());
}
...
void Ruler::draw_hover_mark(QPainter &p, int text_height)
{
	...
	const QPointF points[] = {QPointF(x, b),
