...
Cursor::Cursor(View &view, double time) : TimeMarker(view, FillColour, time)
{
}
...
QString Cursor::get_text() const
{
	...
	return Ruler::format_time_with_distance(diff, time_,
...
QRectF Cursor::label_rect(const QRectF &rect) const
{
	...
	if (time_ > other_time ||
		...
		return QRectF(x - label_size.width(), top, label_size.width(),
			height);
}
...
shared_ptr<Cursor> Cursor::get_other_cursor() const
{
	...
	assert(cursors);
	return (cursors->first().get() == this) ? cursors->second()
						: cursors->first();
}
