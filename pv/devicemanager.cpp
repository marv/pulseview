...
DeviceManager::DeviceManager(shared_ptr<Context> context) : context_(context)
{
	for (auto entry : context->drivers())
		driver_scan(
			entry.second, map<const ConfigKey *, VariantBase>());
}
...
const list<shared_ptr<devices::HardwareDevice>> &DeviceManager::devices() const
{
	return devices_;
}

list<shared_ptr<devices::HardwareDevice>> DeviceManager::driver_scan(
	shared_ptr<Driver> driver, map<const ConfigKey *, VariantBase> drvopts)
{
	...
	devices_.insert(
		devices_.end(), driver_devices.begin(), driver_devices.end());
	devices_.sort(bind(&DeviceManager::compare_devices, this, _1, _2));
	driver_devices.sort(
		bind(&DeviceManager::compare_devices, this, _1, _2));
	...
	const map<string, string> search_info)
{
	...
	for (shared_ptr<devices::HardwareDevice> dev : devices_) {
		...
		if (dev_info.count("connection_id") > 0 &&
			...
			if (dev_info.at("connection_id") ==
				search_info.at("connection_id"))
				return dev;

		// Last resort: vendor/model/version
		if (dev_info.count("version") > 0 &&
			search_info.count("version") > 0)
			if (dev_info.at("version") ==
					search_info.at("version") &&
